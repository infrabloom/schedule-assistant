...
Concrete pitfalls from building the CB4 schedule v1 → v4.15. Read this **before** you start a new DC schedule. Each item is a real failure mode that cost cycles on CB4 — avoid them next time.

---

## 1. OPC Constraint Codes — `CS_MEO` Fails Silently

**What happened on CB4:**
...
**What happened on CB4:**
The prior GC schedule (`Terawulf Cb4-Sum.xer`) had STS procurement appearing twice — once site-wide (`PROC-LL-STS`) and once per DH (`PROC-DH{n}-ELEC-1010`). Carried forward into v4.13 it inflated the procurement WBS and double-counted scope. Same pattern: `MR-2040` Fill/Flush and `MR-5020` DAF Fill & Flush were duplicates of the same scope.
...
**What happened on CB4:**
We had both `MR-2040 Fill/Flush` and `MR-5020 DAF Fill & Flush`. Beyond being duplicates, the **name was wrong**. The convention in mechanical commissioning is: **FLUSH first, then FILL** with treated/inhibited fluid. Mech contractor (MLP) confirmed.
...
**What happened on CB4:**
P6 template defaults gave us 40h for L3-MV-EN, 200h for DH Branch Power install. OCE's detailed mpp showed 420h and 600-1000h for the equivalent scope. The schedule based on template defaults under-stated the critical path by **weeks**.
...
- Document the chosen duration with a citation: "L3-MV-EN 40h — CxA standard per VA-DC template; OCE reports 420h (commissioning of MV switchgear) — held at 40h pending CxA confirmation that scope is single-switchgear test only."
...
Picking B compressed DH4 EFA by ~3 weeks (Sep 14 → Aug 11). Picking A pushed it back. **The answer is contract-specific** — depends on what FluidStack / the owner will accept as "L3 complete."

**Fix:**
Make this a **Phase 3 decision item.** Ask CxA / OCE / CDU vendor directly. Document the interpretation and flag the date impact of the alternative. Track as an open item until resolved.
...
- FluidStack PM said: "All racks physically installed (cabling can lag)."
...
**What happened on CB4:**
Site has East Mains and West Mains (utility tie-ins). It also has A-Feed and B-Feed (redundant power sources for 2N). These are **not the same** — Mains are physical utility connections, Feeds are logical redundant paths to the load.
...
**What happened on CB4:**
The schedule originally had the mech contractor installing CDUs. Patrick flagged: **CDUs are installed by a separate vendor (Schneider / Vertiv)**, not MLP. Same for Fan Walls — DAF procures, but installer is a separate vendor (TBD on CB4).
...
**Fix:**
- Apply per-DH offsets in the activity start dates: DH4 = baseline, DH3 = baseline + 3 weeks, DH2 = baseline + 6 weeks, DH1 = baseline + 9 weeks (CB4 reverse-N-to-S order).
...
## 18. CB4-Specific Pitfalls (Not Portable, But Worth Noting)

These were CB4 mistakes that may not apply to a new project, but capture the pattern:

- **AFEED/BFEED milestones constrained to `CS_MEO`** disappeared on import — see #1.
- **STS shown as both site-wide and per-DH** — see #4.
- **MR-2040 Fill/Flush vs MR-5020 DAF Fill & Flush** duplication — see #4.
- **Rack install set to 0h** initially — caused EFA and FA to land on the same date (impossible — FluidStack needs ~4 weeks to install racks after EFA). Set to 160h.
- **L3-CHW description "Flush, Fill, Water Quality"** misleading — the Flush & Fill is owned by MR-5020 (DAF). L3-CHW is the functional test + water quality verification. **Rename in next version.**
- **Fan Wall installer unspecified** — DAF procures but doesn't install. Open item.
...
# CB4 v2/v3 Rebuild Lessons (added 2026-05-17)
...
**What happened on CB4:**
OCE's MS-Project XML exports use `MinutesPerDay=1440` (24h/day basis) while their
...
a /3 -- but **/3 is specific to OCE's 1440-minute basis; it is NOT universal.**
...
To convert to an 8h/day work-hour basis:
  ...
  - MinutesPerDay = 1440 (24h)  -> factor 1/3   (OCE's case -- the "/3")
  - MinutesPerDay = 480  (8h)   -> factor 1     (MLP's case -- no conversion)
...
## 21. MLP Calendar — No Conversion Needed

**What happened on CB4:**
We initially assumed MLP also needed /3 conversion. It doesn't — MLP's XML uses `MinutesPerDay=480` (8h/day) natively. Don't apply /3 to MLP durations.

**Fix:**
- Check `<MinutesPerDay>` in each XML and apply /3 only if 1440. Don't hardcode the conversion to OCE-source.
...
**What happened on CB4:**
OCE has fine-grained per-area sub-activities (e.g., "Install Cable Tray" appears 16 times across the 4 DH files, plus UPS-East, UPS-West, MECH room, DH proper, sub-rows for Areas A-H). The v3 high-level schedule has one aggregate `MR-1105` per DH covering all of those.

Without an explicit crosswalk, the OCE Refresh produces garbage. We tried auto-matching by name and got <5% high-confidence rate, plus false positives (83 UPS-corridor activities mapped to ADMIN-MEP).

**Fix:**
- Build the OCE/MLP-leaf -> v3-aggregate crosswalk catalog as a Phase 3 deliverable
- One row per v3 task: `v3_code | v3_name | OCE_file | OCE_section_path | OCE_leaf_names | aggregation_rule`
...
**What happened on CB4 v3.0:**
Initial build used `base64.urlsafe_b64encode(uuid.uuid4().bytes)`, which produces GUIDs with `-` and `_`. OPC rejected the XER with a cryptic format error. The CB4 v1.0 lesson #11 said "use base64-encoded UUID" but didn't specify standard vs urlsafe.
...
**What happened on CB4 v3.0:**
The first replication of DH4 -> DH3/2/1 used FS pred chains for crew flow: `CONS-DH4-MR-1010 -> CONS-DH3-MR-1010 -> CONS-DH2-MR-1010 -> CONS-DH1-MR-1010`. This forced serial execution of all 4 DHs by a single crew. But on CB4, multiple crews work in parallel — so DH3 install can start independent of DH4 install, just 21 days later per the stagger.
...
**What happened on CB4 v3.0:**
After Patrick flagged that DH4 EFA had 5 procurement preds but DH3/2/1 EFA had 12, we audited. DH3/2/1 had 12 cross-DH PROC preds (from the LMDC pattern of "DH3 EFA requires DH4 PROC complete") that were wrong — LMDC says each DH's EFA requires its OWN procurement, not other DHs'. Dropped 33 spurious preds and added 12 missing per-DH PROC preds.
...
**What happened on CB4:**
First draft tied `MS-SS-1020` (permanent power milestone) -> `MS-DH4-EFA`. Patrick: "Aren't we using temporary power for that?" Dropped tie. Per CB4 v2.1 definitions: EFA = first access on temp power, FA = full permanent power. They have different power preds.
...
## 31. CDU and Rack Procurement Belong to FluidStack

**What happened on CB4:**
First procurement audit suggested adding `PROC-DH4-CDU` activities. Patrick clarified: CDUs are FluidStack scope; TeraWulf does NOT procure CDUs. Same for racks. Skip these.

**Fix:**
- Confirm scope split in Phase 1 (see kickoff checklist Section B)
- Document the FluidStack-vs-TeraWulf split in the assumptions register
...
- Pattern from CB4: PROC-DH4-CHWPLANT was overdue (exp del 4/8, still 0/5 received per MEL) — flagged in PROC-Verification-List
...
**What happened on CB4 (re-confirmation of lesson #5):**
Confirmed with MLP that the operation is FLUSH-then-FILL. Any activity named "Fill & Flush" or "Fill/Flush" is wrong. Renamed across all DHs.
...
**What happened on CB4:**
OCE's nine XMLs and MLP's XML did not share a consistent outline nesting. DH2's
XML had an extra wrapper level ("CB-4 Data Hall 2" inside "CB-4 Data Hall 2"),
